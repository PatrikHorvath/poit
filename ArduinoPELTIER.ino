  ...
  if (isSystemActive && (millis() - lastTempCheck >= 2000)) {
    ...
    if (celsius != DEVICE_DISCONNECTED_C) {
      if (isSystemActive && !debugActive) {
        ...
        float potentialOutput = (Kp * error) + (Ki * (integralSum + error * Ts)) + (Kd * derivative);

        if (potentialOutput >= 0.0 && potentialOutput <= 100.0) {
          integralSum += error * Ts;
        }
        integralSum = constrain(integralSum, -100.0, 100.0);
